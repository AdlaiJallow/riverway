...
export default function Reservations() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.name || !formData.phone || !formData.address || formData.selectedMenuItems.length === 0) {
      setError('Please fill in all required fields and select at least one menu item.');
      setLoading(false);
      return;
    }

    try {
      // Process order submission via WhatsApp
            ...
            >
              <div className="flex justify-center">
                <button
                  ...
                  className="group relative px-8 sm:px-12 py-4 sm:py-5 bg-gradient-to-r from-green-600 to-green-700 text-white font-bold rounded-xl hover:from-green-700 hover:to-green-800 transition-all duration-300 transform hover:-translate-y-1 hover:shadow-2xl disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-3 overflow-hidden text-base sm:text-lg min-w-[280px]"
                >
                  <span className="relative z-10 flex items-center gap-3">
                    <MessageCircle className="w-6 h-6" />
                    {loading ? 'Sending Order...' : 'Order via WhatsApp'}
                    Order via WhatsApp
                  </span>
