          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              <div className="space-y-4">
                ...
                {['Main Dishes', 'Grilled', 'Signature', 'Beverages'].map(category => {
                        ...
                        {categoryItems.map(item => (
                          ...
                          >
                            <input
                              ...
                              className="w-4 h-4 text-amber-600 bg-gray-100 border-gray-300 rounded focus:ring-amber-500 focus:ring-2 mr-3"
                            />
                            <div className="flex-1">
                              <div className="flex justify-between items-center">
                                <span className="text-sm font-medium">{item.name}</span>
                                <span className="text-sm font-semibold text-amber-700">D{item.price}</span>
                              </div>
                            </div>
                          </label>
                        ))}
              ...
              {formData.selectedMenuItems.length > 0 && (
                <div className="mt-4 p-3 bg-amber-50 border border-amber-200 rounded-lg">
                  <h5 className="text-sm font-medium text-amber-800 mb-2">Selected Items:</h5>
                  <ul className="text-sm text-amber-700">
                    {getSelectedItemNames().map((itemName, index) => (
                      <li key={index}>• {itemName}</li>
                    ))}
                  </ul>
                  <p className="text-xs text-amber-600 mt-2">
                    Final pricing and availability will be confirmed when you arrive.
                  </p>
                </div>
              )}
            </div>
