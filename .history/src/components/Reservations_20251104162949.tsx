...
import { sendReservationEmail, sendWhatsAppNotification, sendWhatsAppMessage } from '../lib/emailService';
...
export default function Reservations() {
  ...
  const handleWhatsAppCall = () => {
    if (!formData.name || !formData.phone || !formData.address || formData.selectedMenuItems.length === 0) {
      setError('Please fill in all required fields and select at least one menu item before placing a call.');
      return;
    }
  ...
  const handleWhatsAppMessage = () => {
    ...
    const totalAmount = calculateTotal();
    const selectedItemsWithPrices = getSelectedItemsWithPrices();
    sendWhatsAppMessage(formData, totalAmount, selectedItemsWithPrices as Array<{name: string, price: number, quantity?: number}>);
